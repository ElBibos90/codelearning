        ...
        try {
            const response = await agent
                ...
                .send({
                    ...
                    status: 'draft'  // Specifica lo stato iniziale
                });
...
afterAll(async () => {
    try {
        // Pulisci i dati di test in ordine inverso di dipendenza
        if (testLesson) {
            await pool.query('DELETE FROM lessons WHERE id = $1', [testLesson.id]);
        }
        if (testCourse) {
            // Questo eliminerà anche le iscrizioni al corso grazie al ON DELETE CASCADE
